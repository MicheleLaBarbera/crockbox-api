...
if(!json_last_error())
{
    ...
    if(property_exists($postData, 'fileName') && property_exists($postData, 'fileExtension') && property_exists($postData, 'fileContent') && property_exists($postData, 'fileSize'))
    {
        ...
        if(file_exists("uploads/$fileName.$fileExtension"))
        {
            /*
            ...
            */
            ...
            do
            {
                ...
                if(file_exists("uploads/$checkFile"))
                    $n++;
                else
                {
        ...
        $query = "INSERT INTO files(name, userid, is_link_generated, size, uploaded_at) VALUES ('$completeFile', 0, 1, '$fileSize', '$uploaded_at')";
        if($mysqli->query($query))
        {
            ...
            if($actualStructure) //vuol dire che è un file XML
            {
                ...
                if($isEqual === true) //vuol dire che ha la stessa struttura del nostro file XML
                {
                    ...
                    if($data != null)
                        file_put_contents("uploads/$completeFile", $data);
                    else
                    {
                ...
                {
                    file_put_contents("uploads/$completeFile", $fileContent);
                }
            }
